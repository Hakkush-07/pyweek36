...
def generate_random_planets(xminmax, yminmax, zminmax, min_distance, f):
    planets = []
    failed = 0
    while True:
        x = uniform(*xminmax)
        ...
        for other in planets:
            if abs(position - other.center) < min_distance:
                far_enough = False
                break
        if far_enough:
            planet = Planet(*position, randint(1, 5), 0.001 * randint(1, 5), random() * 360)
